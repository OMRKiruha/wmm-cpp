...
robustReadMagModelsto compute the magnetic fields. The results are then printed to the specified
output file.
...
int main(int argc, char *argv[]) {
    ...
    wmm::MagneticModel *TimedMagneticModel, *MagneticModels[1];
    wmm::Ellipsoid Ellip{};
    wmm::CoordSpherical CoordSpherical;
    wmm::CoordGeodetic CoordGeodetic;
    wmm::Date UserDate;
    wmm::GeoMagneticElements GeoMagneticElements, Errors;
    wmm::Geoid Geoid;
    char ans[20];
#ifdef WMMHR
    ...
    FILE *coordfile = nullptr, *outfile = nullptr;
    int iline = 0;
    int read_flag;
    ...
    double maxlat    = wmm::LAT_BOUND_MAX;
    double minlat    = wmm::LAT_BOUND_MIN;
    double maxlon    = wmm::LON_BOUND_MAX;
    double minlon    = wmm::LON_BOUND_MIN;
    ...
    void append_errors_to_result_file(FILE * outf, wmm::GeoMagneticElements Errors);

    int coords_header_fmt_size = 30;
    char *coords_header_fmt    = (char *)calloc(coords_header_fmt_size, sizeof(char));
    char *inbuff               = static_cast<char *>(malloc(sizeof(char) * MAXINBUFF));
    int line_size              = 100;
    char *line                 = static_cast<char *>(malloc(sizeof(char) * line_size));
    ...
    TimedMagneticModel = AllocateModelMemory(NumTerms); /* For storing the time modified WMM Model parameters */
    ...
    SetDefaults(&Ellip, &Geoid); /* Set default values and constants */
    ...
    if((argc == 4) && (*(args[1]) == 'f')) {
        ...
        fprintf(outfile, "Date Coord-System Altitude Latitude Longitude D_deg D_min I_deg I_min H_nT X_nT Y_nT Z_nT F_nT "
    ...
    if(argc == 5) {
        ...
        if((*(args[1]) == 'f') || (*(args[2]) == 'f')) {
            ...
            outfile = fopen(out_fname, "w");
            fprintf(outfile, "Date MSL-Ellipsoid Depth(m) Latitude Longitude D_deg D_min I_deg I_min H_nT X_nT Y_nT Z_nT "
                             "F_nT dD_min dI_min dH_nT dX_nT dY_nT dZ_nT dF_nT");
            if(printErrors == 1) {
    ...
    snprintf(coords_header_fmt, coords_header_fmt_size, "%%%ds%%%ds%%%ds%%%ds%%%ds", MAXREAD, MAXREAD, MAXREAD, MAXREAD,
             MAXREAD);
    ...
    while(fgets(line, line_size, coordfile) != NULL && arg_err == 0) {
        ...
        switch(argc) {
            ...
            case 3:
                ...
                inbuff[0] = toupper(inbuff[0]);
                if(inbuff[0] == 'M') {
                    igdgc = 1; /* height is above  mean sea level*/
                } else if(inbuff[0] == 'E') {
                    igdgc = 2; /* height is above  WGS 84 ellepsoid */
                }


            case 2:
                strncpy(inbuff, args[1], MAXREAD);
        ...
        ConvertGeoidToEllipsoidHeight(
            &CoordGeodetic, &Geoid); /*This converts the height above mean sea level to height above the WGS-84 ellipsoid*/
        ...
        if(coords_from_file) {
            ...
            fflush(outfile);
            print_result_file(outfile, GeoMagneticElements.Decl, GeoMagneticElements.Incl, GeoMagneticElements.H,
                              GeoMagneticElements.X, GeoMagneticElements.Y, GeoMagneticElements.Z, GeoMagneticElements.F,
                              60 * GeoMagneticElements.Decldot, 60 * GeoMagneticElements.Incldot, GeoMagneticElements.Hdot,
                              GeoMagneticElements.Xdot, GeoMagneticElements.Ydot, GeoMagneticElements.Zdot,
...
#endif
                append_errors_to_result_file(outfile, Errors);
            }
            fprintf(outfile, "\n");
        }
...
void append_errors_to_result_file(FILE *outf, wmm::GeoMagneticElements Errors) {
    if(std::isnan(Errors.Decl)) {
        if(std::isnan(Errors.X)) {
            fprintf(outf, " NaN         %3.0f  %8.1f      NaN      NaN %8.1f %8.1f", 60 * Errors.Incl, Errors.H, Errors.Z,
                    Errors.F);
        } else {
            fprintf(outf, " NaN         %3.0f  %8.1f %8.1f %8.1f %8.1f %8.1f", 60 * Errors.Incl, Errors.H, Errors.X,
                    Errors.Y, Errors.Z, Errors.F);
        }
    } else {
        fprintf(outf, " %3.0f  %3.0f  %8.1f %8.1f %8.1f %8.1f %8.1f", 60 * Errors.Decl, 60 * Errors.Incl, Errors.H, Errors.X,
                Errors.Y, Errors.Z, Errors.F);
    }
